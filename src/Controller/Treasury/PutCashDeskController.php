...
class PutCashDeskController extends AbstractController
{
    ...
    public function __invoke(mixed $data, Request $request, CashDeskRepository $cashDeskRepository, BranchRepository $branchRepository, UserRepository $userRepository, CurrencyRepository $currencyRepository)
    {
        ...
        {
            return new JsonResponse(['hydra:description' => 'Sorry you are not a branch manager!.'], 400);
        }
        ...
        if ($duplicateCheckOperator && ($duplicateCheckOperator != $data)) {
        ...
        if (isset($cashDeskData['isMain']) && $cashDeskData['isMain']) {
            ...
            if ($isMainCashDesk && ($isMainCashDesk != $data)) {
                return new JsonResponse(['hydra:description' => 'This branch already has a main cashier.'], 400);
            }
        }
