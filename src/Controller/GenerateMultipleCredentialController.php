...
#[AsController]
class GenerateMultipleCredentialController extends AbstractController
{
    private Request $req;
    private EntityManagerInterface $manager;
    public function __construct(EntityManagerInterface $manager, Request $req, private readonly TokenStorageInterface $tokenStorage)
    {
        $this->req = $req;
        $this->manager = $manager;
    }

    public function __invoke(ProfileRepository $profileRepository, InstitutionRepository $institutionRepository, UserPasswordHasherInterface $passwordHasher, UserRepository $userRepository)
    {
        ...
        foreach ($ids as $id){
            ...
            {
                return new JsonResponse(['hydra:title' => $objectName.': '.$objectById->getName().' already has credential'],Response::HTTP_NOT_FOUND, ['Content-Type', 'application/json']);
            }
