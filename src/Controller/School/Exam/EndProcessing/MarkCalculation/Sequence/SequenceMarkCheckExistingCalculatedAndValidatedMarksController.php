...
class SequenceMarkCheckExistingCalculatedAndValidatedMarksController extends AbstractController
{
    ...
    public function checkValidatedMarks(array $students, EvaluationPeriod $evaluationPeriod, Sequence $sequence): array
    {
        ...
        if (!$notMarks) {
            ...
            while ($i < $count && (!$isPossibleToValidate || !$isPossibleToUnvalidate)) {
                ...
                if ($markSequenceCourseCalculated) {
                    ...
                    if ($markSequenceCourseCalculated->getIsValidated()) $isPossibleToUnvalidate = true;
    ...
    #[Route('api/school/mark/sequence/check-validated-marks/{data}', name: 'school_mark_sequence_check_validated_marks')]
    public function checkCalculatedMarksRoute(string $data): JsonResponse
    {
        ...
        $exists = $this->checkValidatedMarks($students, $evaluationPeriod, $sequence);
