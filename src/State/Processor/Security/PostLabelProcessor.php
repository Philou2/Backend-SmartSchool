...
class PostLabelProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = [])
    {
        ...
        if ($data instanceof Menu){
            ...
            if ($position){
                $labelMenu = $this->menuRepository->findOneBy(['module' => $module, 'position' => $position]);
                if ($labelMenu !== null){
            ...
            $menu->setName($donnee->name);
            $menu->setHeadTitle1($donnee->name);
            $menu->setTitle($donnee->title);
            $menu->setType($donnee->title);
            $menu->setPosition($donnee->position);
